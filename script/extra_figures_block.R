...
for (variable in variables) {
  ...
  if (length(datasets) <= 5) {
    shape_fig <- c(length(datasets), 2)
  } else {
    shape_fig <- c(ceiling(length(datasets) / 2), 4)
  }
  ...
  for (dataset in datasets) {
    ...
    if (dataset == datasets[1]) {
      mtext(fp$title_name, side = 3, line = .5, outer = TRUE, cex = 5, font = 2)
    }
    box()
  ...
  for (SECTOR in SECTORS) {
    ...
    for (dataset in datasets) {
      ...
      FREQ <- mean(field_mean, na.rm = T)
      SDFREQ <- sd(field_mean, na.rm = T)
  ...
  for (k in 1:length(SECTORS)) {
    ...
    if (k == 1) {
      mtext(paste0(fp$title_name), side = 3, line = .5, outer = TRUE, cex = 5, font = 2)
    }
...
if (selectname == "nonzero") {
  nonzero <- which(field_ref != 0)
}
if (selectname == "fulldomain") {
  nonzero <- 1:length(field_ref)
}
if (normalize) {
  SIGMA0 <- 1
} else {
...
if (any(variables == "DurationEvents") & any(variables == "NumberEvents")) {
  ...
  for (SECTOR in SECTORS) {
    ...
    if (SECTOR == SECTOR[1]) {
      mtext(paste0("Blocking Events duration vs. Number of events"), side = 3, line = .5, outer = TRUE, cex = 4, font = 2)
    }
