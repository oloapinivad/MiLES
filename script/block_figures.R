...
miles.block.figures <- function(project, dataset, expid, ens, year1, year2,
  ...
  # this will track 3 different blocking indices
  blocking_indices <- c("D12", "ExtraD12", "S04")

  # which fieds to load/plot
  if (varname == "zg") {
    varload <- "Block"
    fieldlist <- c("TM90", "D98", "InstBlock", "AbsBlock", "ExtraBlock", "Z500", "MGI", "BI", "CN", "ACN", "BlockEvents", "DurationEvents", "NumberEvents")
  } else if (varname == "ua") {
    varload <- "U500_Block"
  ...
  for (tracking_index in blocking_indices) {
    ...
    for (field in fieldlist) {
      ...
      nomefile <- file.builder(FILESDIR, varload, paste0(tracking_index, "_Clim"), project, dataset, expid, ens, year1, year2, season)
    ...
    for (field in fieldlist) {
      ...
      if (REFDIR != FILESDIR) {
        nomefile_ref <- file.path(REFDIR, "Block", paste0(paste0(tracking_index, "_Clim_"), dataset_ref, "_", year1_ref, "_", year2_ref, "_", season, ".nc"))
      } else {

        # use file.builder to create the path of the blocking files
        nomefile_ref <- file.builder(FILESDIR, varload, paste0(tracking_index, "_Clim"), project_ref, dataset_ref, expid_ref, ens_ref, year1_ref, year2_ref, season)
      }
    ...
    for (field in fieldlist) {
      ...
      if (field %in% c("MGI", "BI", "CN", "ACN", "BlockEvents", "DurationEvents", "NumberEvents")) {
        outname <- paste0(tracking_index, "_", field)
      } else {
        outname <- field
      }
      figname <- fig.builder(FIGDIR, varload, outname, project, dataset, expid, ens, year1, year2, season, output_file_type)
      print(figname)

      # special treatment for TM90: it is a 1D field!
      if (field == "TM90" | field == "D98") {
        ...
        grid()
        legend("topright", legend = c(info_exp, info_ref), lwd = lwdline, lty = c(1, 1), col = tm90cols, bg = "white", cex = 1.5)
